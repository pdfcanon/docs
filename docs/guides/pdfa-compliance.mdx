...
The `validation.pdfaCompliant` field in `NormalizeResponse` reflects whether the output passed internal compliance checks:
...
For plans that include veraPDF integration, post-normalization PDF/A validation is run using the veraPDF open-source validator (MPL 2.0). Results are reflected in the submission record. Contact support for details on veraPDF-enabled plans.
